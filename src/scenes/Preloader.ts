...
export class Preloader extends Phaser.Scene {
  constructor() {
    const settings: Phaser.Types.Scenes.SettingsConfig = {
      key: "preloader",
      pack: false
    };
    super(settings);
  ...
  loadImages() {
    this.load.setPath(config.imagesPath);
    for (let i = 0; i < config.images.length; i++) {
      this.load.image(config.images[i].name, config.images[i].path);
    }
  }
